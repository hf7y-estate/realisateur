#!/usr/bin/env bash
# zaxon.sh -- the ask_zach and send_zach calls, in ONE place each. crt's
# `demande` verb is the estate-wide door. NEVER FATAL: a recovery that aborts
# because it could not announce itself is worse than a silent one.
# ask_zach is a QUESTION -- it burns the relay's one-at-a-time reply slot.
# send_zach is fire-and-forget -- use it for anything that does not need an
# answer (state-change alerts, etc.), so a flapping caller can't starve
# everyone else's questions (realisateur, 2026-09-09: 113 asks, 0 answers).
...
zaxon_send() {
  local msg="$1" from="${2:-agent}" url hdr sid body ans why
  hdr="$(mktemp)"; body="$(mktemp)"
  for url in ${ZAXON:-http://100.107.253.56:8643/mcp http://127.0.0.1:8643/mcp}; do
    : > "$hdr"
    curl -s -D "$hdr" -o /dev/null --connect-timeout 5 -m 20 -H 'Content-Type: application/json' \
    ...
    curl -s -o /dev/null -m 20 -H 'Content-Type: application/json' \
      -H 'Accept: application/json,text/event-stream' -H "mcp-session-id: $sid" \
      -X POST "$url" -d '{"jsonrpc":"2.0","method":"notifications/initialized"}' >/dev/null 2>&1
    MSG="$msg" FROM="$from" python3 -c 'import json,os; print(json.dumps({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"send_zach","arguments":{"message":os.environ["MSG"],"from_agent":os.environ["FROM"]}}}))' > "$body" || continue
    ans="$(curl -s -m 30 -H 'Content-Type: application/json' \
      -H 'Accept: application/json,text/event-stream' -H "mcp-session-id: $sid" \
      -X POST "$url" --data-binary "@$body" 2>/dev/null | tr -d '\r')"
    why="$(printf '%s' "$ans" | grep -oE '\\?"error\\?": ?\\?"[^"\\]*' | sed 's/.*"//' | head -1)"
    if [ -n "$why" ]; then
      rm -f "$hdr" "$body"
      printf 'zaxon_send: the relay REFUSED this message: %s\n' "$why" >&2; return 0
    fi
    if [ -n "$ans" ]; then rm -f "$hdr" "$body"; printf 'sent\n'; return 0; fi
  done
  rm -f "$hdr" "$body"
  printf 'zaxon_send: no relay answered; the message was NOT delivered\n' >&2
  return 0
}
